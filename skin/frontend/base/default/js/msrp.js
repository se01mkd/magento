...
$(document).observe('bundle:reload-price', function (event) { //reload price
    ...
    if (!Number(bundle.adminhtml.isMAPAppliedDirectly) && !Number(bundle.adminhtml.isFixedPrice)) {
        var canApplyMAP = false;
        try {
            for (var option in bundle.adminhtml.selected) {
                if (bundle.adminhtml.options[option] && bundle.adminhtml.options[option].selections) {
                    var selections = bundle.adminhtml.options[option].selections;
                    for (var i = 0, l = bundle.adminhtml.selected[option].length; i < l; i++) {
                        var selectionId = bundle.adminhtml.selected[option][i];
